...
Append-only record of build sessions. Each entry states what was changed, what was
measured before and after, and what was verified.
...
## Session 2026-07-06 — privacy layer and benchmark honesty

### Baseline verification (re-observed before touching anything)
...
| Git state | `git status` | matches the recorded baseline exactly — 6 modified + `engine/privacy.py`, `tests/test_privacy.py`, audit doc untracked |

Baseline matches the audit's recorded numbers exactly.

### Commit the privacy layer (work-preservation)
...
### Percentile-mode correlation failure under privacy (default path broken)
...
- **Invariant 2 re-verified (determinism):** label-mode `generate(...seed=42,privacy="floored")` output
...
### Privacy scope reconciliation + loud floor-skip

Done together per the audit.
...
- **Discovered + fixed (surfaced, not left silent):** the verbatim-duplicate check over-counted on
...
- `generate` now tracks `floor_applied` + `floor_skip_reason` and surfaces them in the `privacy`
 ...
 `PrivacyReport` (so they flow to `explanation.json` in ). The note text now states plainly when
...
- **Invariant 2 re-verified:** label-mode row-hash still `40933a2bef4f4140` (guard-scope change
...
### Code/repo information protection (done early so later work inherits it)
...
- **(2) No secrets in tracked files.** `TestNoSecretsInTrackedFiles` scans everything `git ls-files`
 ...
 secret-scan hook. Deleted the dead `TABPFN_API_KEY` from `.env`; `.env` was already
...
- **(3) Model-payload redaction seam.** The engine is model-free; documented the provider-agnostic env
 ...
 to send no example values). Actual redaction code lands with Source 3 (item 16); PRIVACY.md
 documents it at .
- **(4) Dataset provenance + gitignore.** Recorded dataset provenance (verified OpenML ids from
 ...
 stale `regen-output/*` files that were committed before the ignore rule (`git rm --cached`, ).
...
- Suite **92 → 94 green**. _(+ handled here; KNOWN_ISSUES header deferred to the
 housekeeping step.)_

### Campaign/screen privacy (decision + plumbing)

- **Decision (surfaced, not left silent).** The audit offered (A) thread privacy through the signatures
...
- **Docs pending (tracked):** `run_campaign` gained params, so the API reference needs the
 new signature — deferred docs pass because will change these same signatures again
 (adding `ScenarioSpec`); updating twice is churn. Docstrings are current now.

### Server + CLI privacy exposure
...
- **Docs:** updated `docs/SERVER_API.md` (§1 generate + §5 campaign) for the request fields and the
...
### Benchmark privacy sweep (floored vs none, 11 datasets)
...
- **Harness:** `benchmark/run_privacy_sweep.py` — fixed config (n_rows=400, seed=7, auto-tune OFF so
 ...
 (`c2ec51a`). Superseded older RESULTS files (now under `benchmark/superseded/`,
 date-stamped) with a header pointing here (kept, not deleted).
...
- **Findings FILED (repro + routed to capability matrix), not silently accepted:**
 ...
 rare_def=None, n_rows=400, auto=False, seed=7, privacy="floored")`. → : **degraded** for
 ...
 categorical TVD vs grounded anchoring. Repro: same call on `open_payments.csv`. → :
 **degraded** for all-categorical high-cardinality; the floor gives no extra protection there
 ...
 modes) — the degeneracy; addressed next.
...
### Lift degeneracy reporting (no more bare 0.0 on tiny rare folds)
...
- **(a)** dead `TABPFN_API_KEY` removed from `.env` — done in .
- **(b)** stale `regen-output/*` untracked, gitignore verified — done in . Confirmed no tracked
...
## Session 2026-07-06 (cont.) — scenario contract, explainability, auditability

### ScenarioSpec contract + manifest round-trip

- **New `contracts/scenario.py`** (pure dataclasses, no engine import): `ColumnSemantics` (the L1
 contract from the semantic-fidelity plan — role/dtype/unit/bounds/categories/integer + per-column
...
### Vetting gate + Sources 1+2 + conformance audit (the differentiator)
...
- **`engine/auditor/conformance.py` — `check_conformance(df, spec)`**: the Auditor's contract gate
 ...
 value-set, identifier uniqueness — reporting violation *counts* (never row values, ). A
...
- **Observed:** `generate` bit-identical (hash `40933a2b`); the example scenario's 5 user
 declarations all vet **accepted**, conformance passes. **done-when demonstrated:** the same
...
### Explainability (`explanation.json`)
...
- **`regen/explain.py` — `build_explanation(...)`**: assembles, from the run's own report objects only
 ...
 (status/n_test_rare/protocol), and the privacy account.
...
- **Observed:** `generate` bit-identical (`40933a2b`); top informative feature `n_prior_txns`
 (Fisher 3.08) — matches finding that it drives the fraud signal; `explanation.json` on disk ==
 `summary["explain"]`.
- **Docs:** `docs/EXPLAINABILITY.md` defines every field + the maintainer rule (new gate/mechanism/
 metric must add its explanation entry).
...
### Independent auditability (audit bundle + `regen verify`)
...
- **`regen/audit_bundle.py::verify_bundle` + CLI `regen verify <dir>`**: pure recomputation from the
 ...
 faked (point 4). When the δ-floor was applied, the gate's correlation was measured pre-floor (not
...
### Standing regression harness + performance budgets
...
- **`benchmark/run_regression.py`**: canonical datasets (creditcard_subset / hypothyroid / wilt) ×
 ...
 . Every scored quantity is compared to committed baselines within explicit tolerances; **exits
...
- **`regen/preflight.py::preflight(path, label_col, rare_def)`** (+ `regen.api.preflight` re-export,
 ...
 non-degenerate lift (`<14` warn, ); all-categorical → floor can't apply; low-cardinality
 integer/ordinal features → floor can collapse coverage (solar_flare); dimensionality > rare
...
### Optional advisory model proposal
...
### Documentation pass (written last, from observed numbers)
...
- **the API reference**: `generate` privacy/delta/scenario/accept_contract params;
 ...
 `regen doctor` surface. (This also clears the doc-pending item — signatures now documented.)
- **Demo (`examples/run_demo.py`)** updated to print explainability highlights (top Fisher-separation
 features), the conformance verdict, and a `regen verify → VERIFIED` line (/done-when).

### Consistency self-review (required before any session is called done)
...
- **repro:** floored corr_delta **0.1011 PASS**, none **0.0478 PASS** — the default path is fixed.
...
**Build complete.** All Part I defects (, , /4/5/6, /8/9/10) and all Part II
workstreams (…) landed as per-finding commits with before/after observations. Known, filed
...
- **Gap screen fixes:** (1) `explanation.json` now **cites metric IDs + versions** from the registry
 (`correlation_delta`/`coverage_rate`/`fisher_separation`/`tail_lift`) — closes the "versioned IDs
...
- **Consequence (surfaced, not hidden):** this exposed that `privacy="floored"` on a **dense percentile
 ...
 downgrade**, which the audit's done-when explicitly sanctions ("pass OR fail loudly with a
...
### TSTR harness (surrogate quality): the headline actionable metric
...
- **Leakage-free orchestration:** `regen.api.evaluate_surrogate` quarantines a real
 ...
 (57a45fc). Degeneracy guard mirrors (`insufficient_real_test` below
...
- **Spec status:** the product spec (now `docs/ARCHITECTURE.md`) TSTR harness **[PLANNED] → [BUILT]**.
...
- **Method note:** worked from the dependency map (blast-radius grep + import sites +
 ...
 (`docs/ARCHITECTURE.md`, `docs/COMPONENT_GUIDE.md`) was updated in step.

---

## Session 2026-07-09 (cont.) — Intent → ScenarioSpec proposer
...
## Session 2026-07-09 (cont.) — Decision-support surface
...
## Session 2026-07-09 (cont.) — Certified-surrogate clean-room demo
 ...
 **conditional-lift** thesis (amplification helps only when the baseline is weak /
 ...
 The breadth results were kept under `benchmark/superseded/` and labelled as pre-leakage-free.

---

## Session 2026-07-11 — Estimand preservation, regression-coefficient v1
