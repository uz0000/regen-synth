...
def main():
    ...
    # The table is written here, by the script that produced it, so no prose
    # file has to restate numbers it cannot recompute.
    md = ["<!-- Generated by examples/certifier_demo/run_demo.py. Do not edit. -->",
          "",
          "# Does the conclusion survive?",
          "",
          f"One logistic regression on {len(real):,} rows of real UCI credit-default "
          f"data, fit on each source and compared to the real fit coefficient by "
          f"coefficient.",
          "",
    ...
    for p_ in PREDICTORS:
        md.append(f"| `{p_}` | {tr[p_]['coef']:+.4f} | "
                  f"[{tr[p_]['ci_low']:+.4f}, {tr[p_]['ci_high']:+.4f}] |")
    md += ["",
           "Per-source, per-coefficient (&#10003; preserved, &#10007; shifted):",
           "",
           "| source | certified | " + " | ".join(f"`{p_}`" for p_ in PREDICTORS) + " |",
           "|---|---|" + "---|" * len(PREDICTORS)]
    for name, cert in certs.items():
        tgt = {t["coefficient"]: t for t in cert.get("targets", [])}
        cells = []
    ...
    n_c = sum(1 for c in certs.values() if c["certified"])
    md += ["",
           f"**{n_c} of {len(certs)} sources certified.** The one that did is a "
           f"resample of the real data, which is the positive control and not "
           f"synthetic at all.",
           ""]
